...
const BASE_PATH = process.env.DESKCOMM_BASE_PATH?.trim().replace(/\/$/, "") ?? "";

function zoneUrl(pathname: string, requestUrl: string) {
  return new URL(`${BASE_PATH}${pathname}`, requestUrl);
}
...
export async function proxy(request: NextRequest) {
  ...
  if (!user) {
    ...
    const loginUrl = zoneUrl("/login", request.url);
  ...
  if (isAdminSurface && pathname.startsWith("/admin") && pathname !== "/admin/forbidden") {
    ...
    if (error || !isAdmin) {
      return NextResponse.redirect(zoneUrl("/admin/forbidden", request.url));
    }
  }
