...
import { ArrowRight, CaretDoubleLeft, CaretDoubleRight, CaretDown, Gear, House } from "@/lib/ui/icons";
...
const OPERATIONAL_HOME = process.env.NEXT_PUBLIC_OPERATIONAL_HOME_URL?.trim() || "/dashboard";
  ...
  return (
    <>
      ...
      <div className="border-b p-2">
        <a
          href={OPERATIONAL_HOME}
          title={collapsed ? t("Voltar ao operacional") : undefined}
          className={cn(
            "flex items-center gap-3 rounded-md px-3 py-1 text-sm text-muted-foreground transition-colors hover:bg-accent/50 hover:text-foreground",
            collapsed && "justify-center px-2",
          )}
        >
          <House size={18} aria-hidden />
          {!collapsed && <span className="truncate">{t("Voltar ao operacional")}</span>}
        </a>
      </div>
      {/*
